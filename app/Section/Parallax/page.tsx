...
export default function Page() {
  ...
  const yAdd = useTransform(smoothProgress, [0, 0.4], [60, 0], {
    clamp: false,
  });
  const oAdd = useTransform(smoothProgress, [0, 0.4], [0, 1], { clamp: false });

  const ySend = useTransform(smoothProgress, [0.25, 0.7], [60, 0], {
    clamp: false,
  });
  const oSend = useTransform(smoothProgress, [0.25, 0.7], [0, 1], {
    clamp: false,
  });

  const yExchange = useTransform(smoothProgress, [0.55, 1], [60, 0], {
    clamp: false,
  });
  const oExchange = useTransform(smoothProgress, [0.55, 1], [0, 1], {
    clamp: false,
  });
        ...
        <div className="min-h-[150vh] overflow-y-hidden mt-80 flex items-center justify-center">
          <div className="flex flex-col items-center pt-0 gap-0 text-center">
            ...
            <motion.h1
              ...
              className="text-6xl md:text-[8rem] font-semibold text-[#477EE9] flex items-center gap-4 md:-mt-16 -mt-10"
            >
            ...
            <motion.h1
              ...
              className="text-6xl md:text-[8rem] font-semibold text-[#FB2D54] flex items-center gap-4 md:-mt-16 -mt-10"
            >
